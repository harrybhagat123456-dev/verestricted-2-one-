...
# License: MIT License
#
# CONVERTED FROM TELETHON TO PYROGRAM — fixes /dl and /adl not working
...
thread_pool = asyncio.get_event_loop().run_in_executor if hasattr(asyncio, 'get_event_loop') else None
ongoing_downloads = {}
...
async def process_audio_pyro(client, message, url, cookies_env_var=None):
    """Process audio download — Pyrogram version."""
    cookies = None
    if cookies_env_var:
    ...
    random_filename = f"@team_spy_pro_{uid}"
    ...
    progress_message = await safe_reply(message, "**__Starting audio extraction...__**")

    try:
        info_dict = await extract_audio_async(ydl_opts, url)
        title = info_dict.get('title', 'Extracted Audio')

        await safe_edit(progress_message, "**__Editing metadata...__**")
        ...
        if os.path.exists(download_path):
            ...
            prog = await safe_send(client, chat_id, "**__Starting Upload...__**")
            ...
            await safe_reply(message, "**__Audio file not found after extraction!__**")

    except Exception as e:
        logger.exception("Error during audio extraction or upload")
        await safe_reply(message, f"**__An error occurred: {e}__**")
    finally:
        if os.path.exists(download_path):
...
# ─── /adl COMMAND ──────────────────────────────────────────────────────────────
@app.on_message(filters.command("adl") & filters.private)
async def adl_handler(client, message):
    """Audio download handler — owner + auth users only."""
    uid = message.from_user.id
    if uid not in OWNER_ID and not await is_auth_user(uid):
        return  # Silently ignore unauthorized users
    
    if uid in ongoing_downloads:
        await safe_reply(message, "**You already have an ongoing download. Please wait until it completes!**")
        return

    if len(message.text.split()) < 2:
        await safe_reply(message, "**Usage:** `/adl <video-link>`\n\nPlease provide a valid video link!")
        return    
    ...
    try:
        if "instagram.com" in url:
            await process_audio_pyro(client, message, url, cookies_env_var="INSTA_COOKIES")
        elif "youtube.com" in url or "youtu.be" in url:
            await process_audio_pyro(client, message, url, cookies_env_var="YT_COOKIES")
        else:
            await process_audio_pyro(client, message, url)
    except Exception as e:
        await safe_reply(message, f"**An error occurred:** `{e}`")
    finally:
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if check_duration_and_size:
            ...
            if duration and duration > 3 * 3600:   
                await safe_edit(progress_message, "**❌ __Video is longer than 3 hours. Download aborted...__**")
                return None

            estimated_size = info_dict.get('filesize_approx', 0)
            if estimated_size and estimated_size > 2 * 1024 * 1024 * 1024:   
                await safe_edit(progress_message, "**🤞 __Video size is larger than 2GB. Aborting download.__**")
                return None
...
async def process_video_pyro(client, message, url, cookies_env_var, check_duration_and_size=False):
    """Process video download — Pyrogram version."""
    ...
    prog = None
    progress_message = await safe_reply(message, "**__Starting download...__**")
    ...
    try:
        ...
        if thumbnail_file:
            ...
            THUMB = await screenshot(download_path, metadata['duration'], uid)
        ...
        if os.path.exists(download_path):
            ...
            await safe_reply(message, "**__File not found after download. Something went wrong!__**")
    except Exception as e:
...
async def dl_handler(client, message):
    """Video download handler — owner + auth users only."""
    uid = message.from_user.id
    if uid not in OWNER_ID and not await is_auth_user(uid):
        return  # Silently ignore unauthorized users
    
    if uid in ongoing_downloads:
        await safe_reply(message, "**You already have an ongoing ytdlp download. Please wait until it completes!**")
        return

    if len(message.text.split()) < 2:
        await safe_reply(message, "**Usage:** `/dl <video-link>`\n\nPlease provide a valid video link!")
        return    
    ...
    try:
        if "instagram.com" in url:
            await process_video_pyro(client, message, url, "INSTA_COOKIES", check_duration_and_size=False)
        elif "youtube.com" in url or "youtu.be" in url:
            await process_video_pyro(client, message, url, "YT_COOKIES", check_duration_and_size=True)
        else:
            await process_video_pyro(client, message, url, None, check_duration_and_size=False)
...
async def split_and_upload_file(app, sender, file_path, caption):
    if not os.path.exists(file_path):
        await safe_send(app, sender, "❌ File not found!")
        return

    file_size = os.path.getsize(file_path)
    start = await safe_send(app, sender, f"ℹ️ File size: {file_size / (1024 * 1024):.2f} MB")
    ...
    async with aiofiles.open(file_path, mode="rb") as f:
        while True:
            ...
            edit = await safe_send(app, sender, f"⬆️ Uploading part {part_number + 1}...")
...
async def progress_bar(current: int, total: int, ud_type: str, message, start: float):
    ...
    if round(diff % 10) == 0 or current == total:
        ...
        try:
            await safe_edit(message,text=f"{ud_type}\n│ {progress_text}")
        except:
            pass
