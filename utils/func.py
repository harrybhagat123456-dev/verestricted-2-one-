...
def E(L):
    ...
    # Bare private channel link with no message ID — treat as starting from msg 1
    bare_private = re.match(r'https://t\.me/c/(\d+)/?$', L)
    public_match = re.match(r'https://t\.me/([^/]+)/(?:\d+/)?(\d+)', L)

    if private_match:
        return f'-100{private_match.group(1)}', int(private_match.group(2)), 'private'
    elif bare_private:
        # User sent just the channel link (no message ID) — default to message 1
        return f'-100{bare_private.group(1)}', 1, 'private'
...
async def screenshot(video: str, duration: int, sender: str) -> str | None:
    ...
    # ── Try ffmpeg first (better quality, precise seeking) ──
    try:
        ...
        if os.path.isfile(output_file):
            ...
            print(f"[VIDEO-THUMB] ffmpeg failed, falling back to cv2...")
    except FileNotFoundError:
        print(f"[VIDEO-THUMB] ffmpeg not found, falling back to cv2...")
    except Exception as e:
        print(f"[VIDEO-THUMB] ffmpeg error: {e}, falling back to cv2...")

    # ── Fallback: cv2 (works without ffmpeg binary) ──
    try:
        ...
        result = await loop.run_in_executor(executor, _cv2_screenshot)
        if result and os.path.isfile(result):
            print(f"[VIDEO-THUMB] cv2 fallback succeeded: {output_file}")
            return result
        else:
            print(f"[VIDEO-THUMB] cv2 fallback also failed")
            return None
    except Exception as e:
        print(f"[VIDEO-THUMB] cv2 fallback error: {e}")
        return None
...
# Auth users can use the bot (batch, single, login, settings, etc.)
# Only the owner can add/remove auth users.
# Non-auth, non-owner users are completely blocked.

async def add_auth_user(user_id: int, added_by: int) -> bool:
    """Add a user to the auth_users collection."""
    try:
...
async def remove_auth_user(user_id: int) -> bool:
    """Remove a user from the auth_users collection."""
    try:
...
async def is_auth_user(user_id: int) -> bool:
    """Check if a user is in the auth_users collection."""
    try:
        user = await auth_users_collection.find_one({"user_id": user_id})
        return user is not None
    except Exception as e:
        logger.error(f"Error checking auth status for {user_id}: {e}")
        return False


async def get_all_auth_users() -> list:
    """Get all auth user IDs."""
    try:
        cursor = auth_users_collection.find({}, {"user_id": 1, "added_by": 1, "added_at": 1})
        users = await cursor.to_list(length=None)
        return users
    except Exception as e:
        logger.error(f"Error getting all auth users: {e}")
        return []
def get_display_name(user):
    if user.first_name and user.last_name:
        return f"{user.first_name} {user.last_name}"
